...
sudo cp ~/devops-engineer-basics/before.rules /etc/ufw/before.rules && \
...
cp ~/devops-engineer-basics/base.conf ~/client-configs/base.conf && \
cp ~/devops-engineer-basics/make_config.sh ~/client-configs/ && chmod +x ~/client-configs/make_config.sh && \
...
sudo cp ~/certs/{ca.crt,vpn.crt,ta.key,vpn.key} /etc/openvpn/server/ && sudo cp ~/devops-engineer-basics/server.conf /etc/openvpn/server/ && sudo chown root:root /etc/openvpn/server/{ca.crt,vpn.crt,ta.key,vpn.key,server.conf} && \
