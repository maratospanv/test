...
gcloud compute scp vpn-server:~/easy-rsa/pki/reqs/client1.req $confdir
gcloud compute scp $confdir/client1.req  pki-server:~/
...
/usr/bin/expect<<EOF
    ...
    expect eof
EOF'\''
gcloud compute scp pki-server:~/easy-rsa/pki/issued/client1.crt $confdir
gcloud compute scp $confdir/client1.crt vpn-server:~/client-configs/keys/
...
gcloud compute scp vpn-server:~/client-configs/files/client1.ovpn $confdir' > $client
